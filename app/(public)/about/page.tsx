...
const CampusMap = nextDynamic(() => import('./CampusMap'), {
  ssr: false,
  loading: () => (
    <div className="flex h-[400px] w-full items-center justify-center rounded-2xl bg-[#080B09] text-xs text-slate-400">
...
export default async function AboutPage() {
  ...
  return (
    <div className="min-h-screen bg-[#1a231d] pb-16">
      {/* Header */}
      <header className="border-b border-[#1e2923] bg-[#121815]/90 backdrop-blur-md shadow-subtle mb-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-bold text-white">Campus Career Copilot</span>
          </Link>
          <Link
            href="/login"
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 space-y-6">
        <div className="rounded-2xl border border-[#1e2923] bg-[#121815] p-8 shadow-card">
          ...
          <h1 className="text-2xl font-extrabold text-white">About Campus Career Copilot</h1>
        ...
        <div className="rounded-2xl border border-[#1e2923] bg-[#121815] p-6 shadow-card">
          ...
          <div className="rounded-2xl overflow-hidden border border-[#1e2923]">
            <CampusMap
            ...
            {visits && visits.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {visits.map((v) => (
                  <div
                    key={v.id}
                    className="flex items-center justify-between rounded-xl border border-[#233028] bg-[#1a231d] p-3 text-xs"
                  >
                    <span className="font-bold text-slate-200">{v.company_name}</span>
                    <span className="rounded bg-[#00D68F]/10 border border-[#00D68F]/10 px-2 py-0.5 text-[11px] font-semibold text-[#00e89b]">
